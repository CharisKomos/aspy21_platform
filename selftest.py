...
def execute(operation: str, **params: object) -> dict:
    res = client.post(
        "/api/execute", json={"operation": operation, "params": params}
    )
...
for path in ("/", "/docs", "/openapi.json", "/api/environment", "/api/catalog",
             "/api/health", "/static/style.css", "/static/app.js"):
...
check(sorted(t["name"] for t in d.get("tags", [])) == ["FLOW_101", "FLOW_102", "FLOW_310"],
      "the * wildcard matches a prefix group")

d = discover(pattern="ATI11?")
check(sorted(t["name"] for t in d.get("tags", [])) == ["ATI111", "ATI112"],
      "the ? wildcard matches exactly one character")

d = discover(description="temperature")
check(d.get("endpoint") == "aspy21_search",
...
check([t["name"] for t in d.get("tags", [])] == ["REACTOR_LEVEL"],
...
r = execute("read_int", tags=pump_tags, start=START, end=END, interval=60,
            output="JSON")
returned = {rec["tag"] for rec in (r.get("records") or [])}
check(returned == set(pump_tags),
...
res = client.post("/api/execute",
                  json={"operation": "read_raw",
                        "params": {"tags": [], "start": START, "end": END}})
check(res.status_code == 400, "empty tag list returns HTTP 400")
check("at least one tag" in res.json().get("detail", "").lower(),
      "the 400 explains what to do")
...
for read_type, extra, calls_for_two_tags in (
    ...
    for output in ("JSON", "DATAFRAME"):
        d = execute(op, tags=["REACTOR_TEMP", "FLOW_101"], start=START, end=END,
                    output=output, **extra)
...
d = execute("read_int", tags=["REACTOR_TEMP", "FLOW_101", "LEVEL_205"],
            start=START, end=END, interval=60, output="JSON")
...
d = execute("read_aggregate", tags=["REACTOR_TEMP", "FLOW_101", "LEVEL_205"],
            start=DAY_START, end=DAY_END, interval=600, output="JSON")
check(d.get("http_call_count") == 3, "AVG issues one XML request per tag")

d = execute("read_snapshot", tags=["REACTOR_TEMP", "ATI111"], include="ALL",
            output="JSON")
...
d = execute("read_int", tags=["REACTOR_TEMP"], start=START, end=END, interval=60,
            output="JSON")
body = (d.get("http_calls") or [{}])[0].get("request_body") or ""
check("period=600" in body, "interval=60 becomes period=600 in the SQL body")
check("request=1" in body, "INT maps to request=1")

d = execute("read_int", tags=["REACTOR_TEMP"], start=START, end=END, interval=None,
            output="JSON")
body = (d.get("http_calls") or [{}])[0].get("request_body") or ""
...
check((d.get("http_calls") or [{}])[0].get("kind") == "Browse",
      "no description filter uses the Browse endpoint")

d = execute("search", tag="*", description="temperature", include="DESCRIPTION",
            limit=100)
check((d.get("http_calls") or [{}])[0].get("kind") == "aspy21_search",
...
check(sorted(d.get("records") or []) == ["ATI111", "ATI112"],
...
d = execute("search_hybrid", tag="REACTOR*", description="", start=START, end=END,
            interval=300, read_type="INT", include="NONE", output="DATAFRAME")
...
d = execute("include_fields", tags=["REACTOR_TEMP"], start=START, end=END,
            variants=["NONE", "STATUS", "DESCRIPTION", "ALL"], output="JSON")
...
d = execute("error_handling", tags=["REACTOR_TEMP"], start=START, end=END,
            read_type="AVG", fail_status="500")
check(d.get("attempts") == 3, "AVG retries 3 times (tenacity on the XML endpoint)")
check("RetryError" in (d.get("error") or {}).get("type", ""),
      "AVG surfaces tenacity.RetryError")

for read_type in ("RAW", "INT", "SNAPSHOT"):
    d = execute("error_handling", tags=["REACTOR_TEMP"], start=START, end=END,
                read_type=read_type, fail_status="503")
    check(d.get("attempts") == 1, f"{read_type} does not retry")
    check("HTTPStatusError" in (d.get("error") or {}).get("type", ""),
...
check(summary.get("total_http_calls") == 2,
      "two identical reads make two HTTP calls (no caching)")
check(summary.get("accepts_cache_kwarg") is False,
      "AspenClient takes no cache= keyword in this release")
check(not any(summary.get("cache_api_probe", {}).values()),
      "no cache methods exist on AspenClient")
